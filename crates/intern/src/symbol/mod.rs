//! Flexible symbol interning, allowing to intern and free strings at runtime while also
...
impl TaggedArcPtr {
    ...
    const fn non_arc(r: &'static &'static str) -> Self {
        ...
        // For the non-arc version the tag is false (0) and thus does not need touching the actual
        // pointer value.
        let packed =
    ...
    pub(crate) unsafe fn try_as_arc_owned(self) -> Option<ManuallyDrop<Arc<Box<str>>>> {
        ...
        if tag != 0 {
            // Safety: We checked that the tag is non-zero -> true, so we are pointing to the data
            // offset of an `Arc`
...
impl Symbol {
    pub fn intern(s: &str) -> Self {
        ...
        //   - if not, put it into a box and then into an Arc, insert it, bump the ref-count and
        //     return the copy
        ...
        let bucket = match shard.find_or_find_insert_slot(
            ...
            Ok(bucket) => bucket,
            // SAFETY: The slot came from `find_or_find_insert_slot()`, and the table wasn't
            // modified since then.
...
mod tests {
    ...
    fn smoke_test() {
        ...
        // "xyzzy" is NOT pre-filled, so it adds a new entry
        let xyzzy = Symbol::intern("xyzzy");
        let _many_worlds = world.clone();
        assert_eq!(MAP.get().unwrap().len(), base_len + 4);
        assert_eq!(
            "hello xyzzy world!",
            format!("{} {} {}{}", hello.as_str(), xyzzy.as_str(), world.as_str(), bang.as_str())
        );
        drop(xyzzy);
        assert_eq!(MAP.get().unwrap().len(), base_len + 3);
        drop(_many_worlds);
