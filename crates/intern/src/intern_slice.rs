...
// SAFETY: This is essentially a `ThinArc`, implemented as a raw pointer because there is no
// `ThinArcBorrowed`.
...
impl<'a, T: SliceInternable> InternedSliceRef<'a, T> {
    ...
    /// # Safety
    ///
    /// You have to make sure the data is not referenced after the refcount reaches zero; beware the
    /// interning map also keeps a reference to the value.
    #[inline]
    ...
    ///
    /// Changes the attached lifetime, as in GC mode, the lifetime is more kind of a lint to prevent
    /// misuse than actual soundness check.
    #[inline]
