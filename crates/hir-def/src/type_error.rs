...
//! to break the hir-ty -> ide-diagnostics cycle.

use crate::message::{Message, MessageSeverity};
use parser::Span;
...
pub enum TypeError {
    NoOverloading { id: String, errors: Vec<(String, Span, Box<TypeError>)> },
    UnresolvedQuants { id: String, quants: Vec<String>, signature: Option<String> },
    FailedConstraint { constraint: String, derived_from: Vec<Span> },
    Subtype { lhs: String, rhs: String, constraint: Option<String> },
    Other(String),
    FunctionArg { span: Span, ty: String, error: Box<TypeError> },
    VectorSubrange { first: String, second: String, order: VectorOrder },
    Hint(String),
    Alternate { primary: Box<TypeError>, reasons: Vec<(String, Span, Box<TypeError>)> },
}
    ...
    pub fn message(&self) -> (Message, Option<String>) {
        match self {
            TypeError::NoOverloading { id, errors } => {
                let list = errors
                    ...
                    .map(|(candidate, span, error)| {
                        ...
                        (candidate.clone(), Message::location(String::new(), hint, *span, message))
                    })
                    .collect::<Vec<_>>();
                (
                    Message::seq([
            ...
            TypeError::FailedConstraint { constraint, derived_from } => {
                let mut messages =
                    vec![Message::line(format!("Failed to prove constraint: {constraint}"))];
            ...
            TypeError::Subtype { lhs, rhs, constraint } => {
                let mut messages = vec![Message::Severity(
                    MessageSeverity::Warning,
                    Box::new(Message::line(format!("{lhs} is not a subtype of {rhs}"))),
                )];
                if let Some(constraint) = constraint {
                    messages.push(Message::line(format!("as {constraint} could not be proven")));
                }
                (Message::Seq(messages), None)
            }
            ...
            TypeError::VectorSubrange { first, second, order } => {
            ...
            TypeError::Alternate { primary, reasons } => {
                ...
                let reasons = reasons
                    ...
                    .map(|(header, span, error)| {
                        let (message, hint) = error.message();
                        (header.clone(), Message::location(String::new(), hint, *span, message))
                    })
                    .collect::<Vec<_>>();
