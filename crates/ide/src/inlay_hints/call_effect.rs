...
pub(super) fn collect_call_effect_hints(
    ...
    let fallback_effects: std::collections::HashMap<String, std::collections::BTreeSet<hir_def::EffectTag>>;
    let effects_map: &std::collections::HashMap<String, std::collections::BTreeSet<hir_def::EffectTag>> =
    ...
    for entry in bodies.entries() {
        for (id, hir) in entry.body.iter_exprs() {
            let call_span =
                entry.source_map.expr_syntax(id).unwrap_or(parser::Span::new(0, 0));
