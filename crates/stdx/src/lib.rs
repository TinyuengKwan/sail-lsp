...
/// Mirrors rust-analyzer's assert_eq_text! macro.
#[macro_export]
macro_rules! assert_eq_text {
    ($left:expr, $right:expr) => { $crate::assert_eq_text!($left, $right,) };
    ($left:expr, $right:expr, $($tt:tt)*) => {{
        let left = $left;
        let right = $right;
        if left != right {
            if left.trim() == right.trim() {
                eprintln!("Left:\n{:?}\n\nRight:\n{:?}\n\nWhitespace difference", left, right);
            } else {
                let left_ref: &str = left.as_ref();
                let right_ref: &str = right.as_ref();
                let diff = $crate::diff::__diff(left_ref, right_ref);
                eprintln!("Left:\n{}\n\nRight:\n{}\n\nDiff:\n{}", left, right, $crate::diff::format_diff(diff));
            }
            eprintln!($($tt)*);
            panic!("text differs");
        }
...
/// Guard that panics on drop unless defused. Used to ensure
/// every started marker/node is either completed or abandoned.
///
/// Note: RA does not have `DropBomb` in stdx; it lives elsewhere.
/// We keep it here because sail-lsp's parser depends on it.
pub struct DropBomb {
    defused: bool,
    msg: &'static str,
}
...
"
        ...
        assert_eq_text!(trim_indent("    hello\n    world\n"), "hello\nworld\n");
        assert_eq_text!(
            trim_indent(
                "
            fn main() {
    ...
    #[test]
    fn test_dedent() {
        assert_eq_text!(dedent_by(0, ""), "");
        assert_eq_text!(dedent_by(1, ""), "");
        assert_eq_text!(dedent_by(2, ""), "");
        assert_eq_text!(dedent_by(0, "foo"), "foo");
        assert_eq_text!(dedent_by(2, "foo"), "foo");
        assert_eq_text!(dedent_by(2, "  foo"), "foo");
        assert_eq_text!(dedent_by(2, "    foo"), "  foo");
        assert_eq_text!(dedent_by(2, "    foo\nbar"), "  foo\nbar");
        assert_eq_text!(dedent_by(2, "foo\n    bar"), "foo\n  bar");
        assert_eq_text!(dedent_by(2, "foo\n\n    bar"), "foo\n\n  bar");
        assert_eq_text!(dedent_by(2, "foo\n.\n    bar"), "foo\n.\n  bar");
        assert_eq_text!(dedent_by(2, "foo\n .\n    bar"), "foo\n.\n  bar");
